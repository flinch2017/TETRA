...
router.post('/api/like-track', async (req, res) => {
  ...
  try {
    if (liked) {
      await pool.query(
        'INSERT INTO likes (like_id, acode, track_id) VALUES ($1, $2, $3) ON CONFLICT DO NOTHING',
        [crypto.randomUUID(), acode, track_id]
      );
    } else {
      await pool.query(
        'DELETE FROM likes WHERE acode = $1 AND track_id = $2',
        [acode, track_id]
      );
    }
